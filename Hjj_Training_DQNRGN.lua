...
-- define loss function and trainer for dqn
...
do
	...
	print('It is the ' .. i .. ' epoch, max_epchs = '.. max_epochs)
	print(training_clip_table)
	...
	do
		...
		do
			...
			print('\t\t\tInit mask: ' .. cur_mask[1] .. '\t' .. cur_mask[2] .. '\n')			
			-- get C3D
			--local C3D_vector = func_get_C3D(opt.data_path, opt.class, 1,
			--								 v, cur_mask[1], cur_mask[2], C3D_m, {})
			local C3D_vector = func_get_C3D(clip_img[{ {cur_mask[1], cur_mask[2]},{} }], C3D_m,{})
			...
			do
				...
				if action == trigger_action then -- estemated as trigger
					...
					overlap = func_calculate_overlapping(now_target_gt, cur_mask)
						...
						if count_batch > 0 then
							...
							tmp_mod = tmp_mod[1]
								...
								if tmp_mod == 0 or optimState_rgn.evalCounter == 1 then
									local mdl_name={}
									if opt.gpu >= 0 then
										mdl_name = './model/rgn_'.. opt.name .. opt.class .. '_'.. optimState_rgn.evalCounter
									else 
	...
	print('end this epoch :' .. i ..'  , max_epochs ' .. max_epochs .. '\n')
end -- epochs loop
